...
import { CustomerData } from '@/services/predictionService';
...
export function PredictionRow({ no, data, onViewClick }: PredictionRowProps) {
  ...
  const barColor = score && score > 70 ? 'bg-red-500' : score && score > 40 ? 'bg-yellow-400' : 'bg-green-500';
    ...
    <tr className="border-b border-gray-50 hover:bg-gray-50/50 transition">
      ...
      <td className="py-4">
        <div className="flex items-center justify-center gap-3">
          ...
          {score && score > 50 ? <ArrowUp size={12} className="text-red-500" /> : <ArrowDown size={12} className="text-green-500" />}
