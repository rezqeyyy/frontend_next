...
export function useCsvUpload() {
  ...
  const parseAndUpload = (file: File) => {
    setIsUploading(true);
    setStatus('idle');
      ...
      complete: async (results) => {
        try {
            const validData = results.data.filter((row: any) => 
                (row.name && row.name.trim() !== '') || (row.customer_id && row.customer_id.trim() !== '')
            );

            if (validData.length === 0) throw new Error("Tidak ada data valid.");

          // src/hooks/useCsvUpload.ts (Bagian Mapping Data)
            const formattedData = validData.map((row: any) => {
            // Fungsi helper biar gak ngetik Number() berulang kali
            const n = (val: any) => (val !== undefined && val !== null && val !== '') ? Number(val) : 0;

            return {
                customer_id: row.customer_id, // Primary Key
          ...
          await customerService.upsertCustomers(formattedData);
