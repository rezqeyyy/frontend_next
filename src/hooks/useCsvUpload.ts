...
export function useCsvUpload() {
    ...
    try {
        // 1. CEK SESSION: Pastiin user beneran masih login
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) throw new Error("Session lu ilang jink, coba relogin.");

        // 2. BIKIN FOLDER (Dataset)
        ...
        // 3. PARSE CSV (Pake Promise biar beneran ditungguin)
        const formattedData = await new Promise<any[]>((resolve, reject) => {
            ...
            complete: (results) => {
                ...
                .map((row: any) => ({
                    ...
                    // MAPPING EKSPLISIT: Biar gak ada kolom sampah dari CSV yang masuk
        ...
        // 4. UPLOAD DATA (Sekarang beneran ditungguin)
        ...
        setStatus('success');
        setMessage(`Berhasil! ${formattedData.length} data masuk ke folder lu.`);
